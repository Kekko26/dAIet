...
import {
  ...
  ADAPT_TO_MEDICATIONS_PROMPT,
  DIET_GENERATOR_OUTPUT_EXAMPLE,
  DIET_GENERATOR_PROMPT,
} from './prompt';
...
export class DietGeneratorService {
  ...
  public async failSafeDietGeneration(data: UserParameters): Promise<any> {
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        console.error(
          `Error while generating diet, attempt ${attempt + 1}`,
          error,
        );
        if (attempt === 2) {
          return {
  ...
  async _generateDiet(data: UserParameters) {
    ...
    const prompt = await template.format({
      ...
      goal: data.goal,
      food_to_avoid:
        data?.diseases?.map((d) => d.restricted_foods).join(', ') +
        ', ' +
        data?.allergens,
    });
  ...
  private async _insertMedicine(data: {
    medications: MedicationDTO[];
    dietPlan: DietPlan;
  }) {
    const medStr = data.medications
      .map((medication) => this._parseMedication(medication))
      .join('\n');
  ...
  private _parseMedication(medication: MedicationDTO) {
    return `
